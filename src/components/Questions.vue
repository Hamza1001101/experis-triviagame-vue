<template>
  <div>
    ...
    <div v-if="questions.results[index].type === 'multiple'">
      <button
        id="0"
        @click="getNextQuestion($event)"
        v-html="questionsArr[0]"
      ></button>
      <button
        id="1"
        @click="getNextQuestion($event)"
        v-html="questionsArr[1]"
      ></button>
      <button
        id="2"
        @click="getNextQuestion($event)"
        v-html="questionsArr[2]"
      ></button>
      <button
        id="3"
        @click="getNextQuestion($event)"
        v-html="questionsArr[3]"
      ></button>
    </div>
    <div v-else>
...
export default {
      ...
      if (this.index === this.attributesQuiz.amount) {
        ...
        this.$router.push({
          ...
          params: {
            ...
            attributesQuiz: this.attributesQuiz,
  ...
  methods: {
    ...
    getNextQuestion: function (e) {
      ...
      this.questions.results[this.index].correct_answer = this.decodeHtml(
        this.questions.results[this.index].correct_answer
      );
      this.index++;
    ...
    addAnswersToQuestionArray() {
      if (this.questions.results[this.index].type === "multiple") {
        let answers = [
          ...this.questions.results[this.index].incorrect_answers,
          this.questions.results[this.index].correct_answer,
        ];
